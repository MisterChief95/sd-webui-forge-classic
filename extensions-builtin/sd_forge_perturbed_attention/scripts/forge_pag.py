...
class PerturbedAttentionGuidanceForForge(scripts.Script):
    ...
    def ui(self, is_img2img):
        with InputAccordion(False, label=self.title()) as enabled:
            ...
            with gr.Row():
                skip_hires = gr.Checkbox(label='Skip HiRes Fix', value=False, visible=not is_img2img)
        ...
        self.infotext_fields = [
            ...
            (skip_hires, "pagi_skip_hires"),
        ]

        return enabled, scale, attenuation, start_step, end_step, skip_hires
    ...
    def process_before_every_sampling(self, p, *script_args, **kwargs):
        enabled, scale, attenuation, start_step, end_step, skip_hires = script_args

        if not enabled or (getattr(p, 'is_hr_pass', False) and not skip_hires):
            return
        ...
        def post_cfg_function(args):
            ...
            model, cond_denoised, cond, sigma, x, options = \
                args["model"], args["cond_denoised"], args["cond"], args["sigma"], args["input"], args[
                    "model_options"].copy()
